...
export const buildReplayDatasetAnalysis = (datasetId: string, symbol: string, bars1m: OhlcvBar[]): ReplayDatasetAnalysis => {
  ...
  if (d2) {
    ...
    eventLog.push(makeEvent('background', dump ? 'Dump Day detected' : pump ? 'Pump Day detected' : 'Background invalid', `${strategyNyDate(strategyTime(d2))} background classified from daily structure.`, dump ? 'Range/body/close structure shows dump-day momentum.' : pump ? 'Range/body/close structure shows pump-day momentum.' : 'D-2 did not establish pump or dump background.', 0, trace, strategyTime(d2)));
  }
  if (d1) {
    ...
    const trace: RuleTraceItem[] = [{ ruleName: 'Signal day', timeframe: '1D', passed: fgd || frd, reason: fgd ? 'FGD detected from dump background and bullish D-1 close.' : frd ? 'FRD detected from pump background and bearish D-1 close.' : 'D-1 does not complete FRD/FGD signal rules.', prices: { bodyPips: body, rangePips: range }, times: { day: strategyNyDate(strategyTime(d1)), sourceStart: d1.sourceStartTime ?? d1.sourceTime ?? d1.time, sourceEnd: d1.sourceEndTime ?? d1.sourceTime ?? d1.time } }, { ruleName: 'Signal quality', timeframe: '1D', passed: template === 'FGD' ? body >= 40 && body / Math.max(range, 1) >= 0.6 : inside, reason: template === 'FGD' ? 'FGD priority checks body >= 40 pips and >= 60% of range.' : 'FRD priority checks inside day vs D-2.', prices: { bodyPips: body, rangePips: range, d2High: d2?.high ?? 0, d2Low: d2?.low ?? 0 }, times: { day: strategyNyDate(strategyTime(d1)), sourceStart: d1.sourceStartTime ?? d1.sourceTime ?? d1.time, sourceEnd: d1.sourceEndTime ?? d1.sourceTime ?? d1.time } }];
    ruleTrace.push(...trace);
    eventLog.push(makeEvent('signal', fgd ? 'FGD detected' : frd ? 'FRD detected' : 'Signal day incomplete', `${strategyNyDate(strategyTime(d1))} signal day assessed.`, fgd ? 'FGD detected — dump background + bullish signal body. Next: watch New York LOS source and low sweep reversal.' : frd ? 'FRD detected — inside day + bearish close. Next: watch New York HOS source and reclaim failure.' : 'Signal day is present but template is incomplete.', 1, trace, strategyTime(d1)));
  }

  const startIndex = bars1m.findIndex((bar) => strategyNyDate(strategyTime(bar)) === tradeDay && inNySession(bar));
  ...
  if (session.length) {
    ...
    if (sourceIndex !== -1) {
      ...
      const sourceBar = session[sourceIndex];
      const trace = [{ ruleName: 'Source', timeframe: '1m', passed: true, reason: template === 'FGD' ? 'FGD source uses LOS sweep.' : 'FRD source uses HOS sweep.', prices: { source: sourcePrice ?? 0, previousClose: previousClose ?? 0, distanceToPreviousClosePips: nearPrev ?? -1 }, times: { strategyBar: strategyTime(sourceBar), sourceBar: sourceBar.sourceTime ?? sourceBar.time } } satisfies RuleTraceItem];
      ruleTrace.push(...trace);
      eventLog.push(makeEvent('source', template === 'FGD' ? 'LOS source detected' : 'HOS source detected', 'Source level appeared during Day 3 session.', `${template === 'FGD' ? 'LOS' : 'HOS'} source formed ${nearPrev !== undefined ? `with previous-close distance ${nearPrev} pips.` : 'without previous close reference.'}`, global, trace, strategyTime(sourceBar), { source: sourcePrice ?? 0 }));
      annotations.push(makeAnnotation('source', global, strategyTime(sourceBar), sourcePrice ?? 0, 'Source', 'Source level used for Day 3 setup.', trace));
      stopPrice = template === 'FGD' ? (sourcePrice ?? 0) - 0.0021 : (sourcePrice ?? 0) + 0.0021;
      annotations.push(makeAnnotation('stop', global, strategyTime(sourceBar), stopPrice, 'Stop', 'Stop sits outside source extreme; >20 pips means skip.', trace));
    }
    if (stopHuntIndex !== -1) {
      const global = replayStartIndex + stopHuntIndex;
      const stopHuntBar = session[stopHuntIndex];
      const sourceBar = session[sourceIndex];
      const trace = [{ ruleName: 'Stop hunt', timeframe: '1m', passed: true, reason: template === 'FGD' ? 'Price swept the prior low and reclaimed above it quickly.' : 'Price swept the prior high and reclaimed below it quickly.', prices: { source: sourcePrice ?? 0, reclaim: stopHuntBar.close }, times: { strategySweep: strategyTime(sourceBar), sourceSweep: sourceBar.sourceTime ?? sourceBar.time, strategyReclaim: strategyTime(stopHuntBar), sourceReclaim: stopHuntBar.sourceTime ?? stopHuntBar.time } } satisfies RuleTraceItem];
      ruleTrace.push(...trace);
      eventLog.push(makeEvent('stop-hunt', 'Stop hunt confirmed', 'Stop hunt rule passed.', template === 'FGD' ? 'Stop hunt confirmed — low sweep reclaimed above the swept level.' : 'Stop hunt confirmed — high sweep reclaimed back below the swept level.', global, trace, strategyTime(stopHuntBar)));
      annotations.push(makeAnnotation('stopHunt', global, strategyTime(stopHuntBar), stopHuntBar.close, 'Stop hunt', 'Quick reclaim after source sweep.', trace));
      ...
      eventLog.push(makeEvent('stop-hunt', 'Stop hunt not confirmed', 'Stop hunt rule failed so far.', 'Need a sweep and quick reclaim before upgrading setup quality.', replayStartIndex + Math.max(sourceIndex, 1), [], session[Math.max(sourceIndex, 1)] ? strategyTime(session[Math.max(sourceIndex, 1)]) : undefined));
    }
    if (p1 !== -1) {
      const trace = [{ ruleName: '123 structure', timeframe: '1m', passed: true, reason: '1-2-3 reversal mapped after stop hunt.', prices: { node1: session[p1].close, node2: session[p2].close, node3: session[p3].close, breakout: session[breakout].close }, times: { node1: strategyTime(session[p1]), node2: strategyTime(session[p2]), node3: strategyTime(session[p3]), breakout: strategyTime(session[breakout]), sourceNode1: session[p1].sourceTime ?? session[p1].time, sourceNode2: session[p2].sourceTime ?? session[p2].time, sourceNode3: session[p3].sourceTime ?? session[p3].time, sourceBreakout: session[breakout].sourceTime ?? session[breakout].time } } satisfies RuleTraceItem];
      ruleTrace.push(...trace);
      eventLog.push(makeEvent('pattern-123', '123 structure ready', 'Valid 1-2-3 structure is available.', '123 is complete and the breakout leg is visible.', replayStartIndex + breakout, trace, strategyTime(session[breakout])));
      annotations.push(makeAnnotation('point1', replayStartIndex + p1, strategyTime(session[p1]), session[p1].close, '1', '123 node 1', trace));
      annotations.push(makeAnnotation('point2', replayStartIndex + p2, strategyTime(session[p2]), session[p2].close, '2', '123 node 2', trace));
      annotations.push(makeAnnotation('point3', replayStartIndex + p3, strategyTime(session[p3]), session[p3].close, '3', '123 node 3', trace));
    } else {
    ...
    if (five.length) {
      ...
      if (emaIndex !== -1) {
        ...
        const global = bars1m.findIndex((bar) => strategyTime(bar) === strategyTime(anchorBar));
        const trace = [{ ruleName: '20EMA confirm', timeframe: '5m', passed: true, reason: template === 'FGD' ? '5m close back above 20EMA.' : '5m close back below 20EMA.', prices: { close: anchorBar.close, ema20: ema5[emaIndex] }, times: { strategyBar: strategyTime(anchorBar), sourceStart: anchorBar.sourceStartTime ?? anchorBar.sourceTime ?? anchorBar.time, sourceEnd: anchorBar.sourceEndTime ?? anchorBar.sourceTime ?? anchorBar.time } } satisfies RuleTraceItem];
        ruleTrace.push(...trace);
        eventLog.push(makeEvent('ema', '20EMA confirm', 'Momentum returned through the 20EMA gate.', trace[0].reason, global, trace, strategyTime(anchorBar)));
        annotations.push(makeAnnotation('ema', global, strategyTime(anchorBar), anchorBar.close, '20EMA', '5m EMA confirmation.', trace));
      } else {
        missingConditions.push('20EMA confirm pending.');
      }
    }
    if (p1 !== -1 && emaIndex !== -1 && sourcePrice !== undefined) {
      entryIndex = Math.max(replayStartIndex + breakout, bars1m.findIndex((bar) => strategyTime(bar) === strategyTime(five[emaIndex])));
      ...
      const trace = [{ ruleName: 'Entry gate', timeframe: '1m', passed: stopDistance <= 20, reason: stopDistance <= 20 ? 'Entry valid with stop <= 20 pips.' : 'Skip: stop too large.', prices: { entry: entryPrice ?? 0, stop: stopPrice ?? 0, stopDistance }, times: { strategyEntry: strategyTime(bars1m[entryIndex] ?? { time: '' }), sourceEntry: bars1m[entryIndex]?.sourceTime ?? bars1m[entryIndex]?.time ?? '' } } satisfies RuleTraceItem];
      ruleTrace.push(...trace);
      eventLog.push(makeEvent('entry', stopDistance <= 20 ? 'Entry valid' : 'Skip: stop too large', 'Entry gate evaluated.', stopDistance <= 20 ? 'Entry valid — source, stop hunt, 123, and EMA gates align.' : 'Skip: stop too large', entryIndex, trace, strategyTime(bars1m[entryIndex] ?? session[0])));
      annotations.push(makeAnnotation('entry', entryIndex, strategyTime(bars1m[entryIndex] ?? session[0]), entryPrice ?? 0, 'Entry', 'Entry becomes valid only after all gates align.', trace));
    }
  }
