...
export const validateDataset = (bars: OhlcvBar[]): DatasetValidationIssue[] => {
  ...
  if (aggregateBars(bars, '5m').length === 0 || aggregateBars(bars, '15m').length === 0 || aggregateBars(bars, '1D').length < 3) {
    issues.push({ code: 'timeframe-discontinuity', message: 'Invalid dataset: unable to validate FRD/FGD template', detail: 'Could not rebuild one or more required timeframes from the normalized strategy timeline.' });
  }
...
export const getTradeDay = (bars: OhlcvBar[]) => Object.keys(byNyDate(bars)).sort().slice(-1)[0] ?? strategyNyDate(strategyTime(bars[0] ?? { time: new Date().toISOString() }));
