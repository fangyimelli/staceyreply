...
export function renderCandlestickChart(
  ...
  const priceToY = (p: number) => h - pad - ((p - minPrice) / (maxPrice - minPrice || 1)) * (h - pad * 2);
  ...
  candles.forEach((c, i) => {
    const x = pad + ((w - pad * 2) / candles.length) * i;
  ...
  const drawHLine = (price: number, color: string, label: string) => {
    ...
    ctx.setLineDash([]);
    ctx.fillStyle = color;
    ctx.fillText(label, w - pad + 4, y + 3);
  };

  ctx.strokeStyle = "#f59e0b";
  ctx.beginPath();
  result.overlays.ema20.forEach((p, i) => {
    const x = pad + ((w - pad * 2) / candles.length) * i;
    const y = priceToY(p);
  ...
  drawHLine(result.overlays.previousClose, "#94a3b8", "prev close");
  drawHLine(result.overlays.hos, "#38bdf8", "HOS");
  drawHLine(result.overlays.los, "#0ea5e9", "LOS");
  drawHLine(result.overlays.hod, "#eab308", "HOD");
  drawHLine(result.overlays.lod, "#f97316", "LOD");

  const markerPoints = result.markers.map((m, idx) => {
    const x = pad + ((w - pad * 2) / candles.length) * Math.max(0, candles.length - 1 - idx);
  ...
  canvas.onmousemove = (event) => {
    ...
    const hit = markerPoints.find((m) => Math.hypot(mx - m.x, my - m.y) < 8);
    if (!hit) {
