...
const pip = (a, b) => (a - b) * 10000;
const day = (t) => t.slice(0, 10);
const hmNy = (t) => new Intl.DateTimeFormat('en-US', { timeZone: 'America/New_York', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(t));
export const detectCandidates = (symbol, bars1m) => {
    const d = aggregate(bars1m, '1D');
    const out = [];
    for (let i = 2; i < d.length; i++) {
        const d2 = d[i - 2], d1 = d[i - 1], d3 = d[i];
        const body = Math.abs(pip(d1.close, d1.open));
        const range = Math.abs(pip(d1.high, d1.low));
        if (d2.close < d2.open && d1.close > d1.open && body >= 40 && body >= 0.6 * range)
            out.push({ symbol, date: day(d3.time), type: 'FGD', reason: 'D-2 dump + D-1 bullish' });
        if (d2.close > d2.open && d1.close < d1.open && d1.high <= d2.high && d1.low >= d2.low)
            out.push({ symbol, date: day(d3.time), type: 'FRD', reason: 'D-2 pump + D-1 bearish inside' });
    }
    return out;
};
export const evaluate = (line, bars5m, d, mode, mEntry, mExit) => {
    ...
    const ix = b.findIndex((x) => x.time === srcBar.time);
    const m30 = b.slice(ix, ix + 6);
    ...
    let target = null;
    if (emaConfirm && move30 >= 15)
        target = 30;
    if (target && move30 >= 30)
        target = 35;
    if (target && move30 >= 30 && (stopHunt || engulf))
        target = 40;
    if (stopHunt && oneTwoThree && emaConfirm && move30 >= 35)
        target = 50;
    const entry = line === 'FGD' ? p2.high : p2.low, stop = line === 'FGD' ? p1.low : p1.high;
    const entryAllowed = Boolean(target) && Math.abs(pip(entry, stop)) <= 20;
    let trade;
    if (mode === 'auto' && entryAllowed) {
        const tp = target ? (line === 'FGD' ? entry + target / 10000 : entry - target / 10000) : entry;
        const ex = b[b.length - 1].close;
        trade = { side: line === 'FGD' ? 'LONG' : 'SHORT', entry, exit: tp ?? ex, pnlPips: line === 'FGD' ? pip(tp ?? ex, entry) : pip(entry, tp ?? ex) };
    }
    if (mode === 'manual' && Number.isFinite(mEntry) && Number.isFinite(mExit) && mEntry !== undefined && mExit !== undefined) {
        trade = { side: line === 'FGD' ? 'LONG' : 'SHORT', entry: mEntry, exit: mExit, pnlPips: line === 'FGD' ? pip(mExit, mEntry) : pip(mEntry, mExit) };
    }
    const explain = { template: line, bias: line === 'FGD' ? 'LONG' : 'SHORT', stage: entryAllowed ? 'Entry Qualified' : 'Building setup', missing: [!emaConfirm ? '20EMA confirm missing' : '', !stopHunt ? 'stop hunt missing' : '', !oneTwoThree ? '123 missing' : '', Math.abs(pip(entry, stop)) > 20 ? 'skip: stop too large' : ''].filter(Boolean), reasons: [`source=${line === 'FGD' ? 'LOS' : 'HOS'}`, `move30=${move30.toFixed(1)} pips`, `rotation from ${hmNy(srcBar.time)} near :00/:15/:30/:45`], entryAllowed, target };
    const annotations = [
